...
DLT大乐透智能预测系统 V2.1.0。六池采样（热号/冷号/均衡/博弈/遗传/趋势）+ 博弈论遗传算法融合 + 约束满足引擎 + 跨期模式识别 + **尾号聚合检测 + AC值跟踪 + 偏差仪表盘校准**，支持前后区复式投注生成。
...
- **V2.1.0** (2026-06-02): 基于26060期对比分析的五大优化
  - 1. 趋势池修正 — 新增generate_trend_pool()，基于和值趋势修正号码权重，六池采样(热30%+冷15%+均衡20%+趋势20%+博弈10%+遗传5%)
  - 2a. 删除固定重号预期 — 改为动态判断(和值>120→无重号预期, <70→少重号, 中等→正常)
  - 2b. 奇偶约束放宽 — 新增策略6小众均衡策略，允许全偶(0:5)/全奇(5:0)采样路径
  - 3a. 尾号聚合检测 — 尾号≤3个降3%，尾号4个不同(1个重复)→加1%
  - 3b. AC值跟踪评分 — 基于历史100期AC值概率分布调整候选
  - 4a. 偏差仪表盘 — 自动跟踪和值/跨度/大小号偏差
  - 4b. 置信度重校准 — 偏差过大时补偿覆盖被低估方向的候选
